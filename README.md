...
- **Size Intelligence (Tabela Horeca) & Global Concurrency Lock**: Czysty UI i koniec z domyślaniem się rozmiaru pracowniczego. W komponencie `TryOnWidget.tsx` usunięto klasyczny wybór rozmiarów (S, M, L) na rzecz pola **"Obwód Klatki Piersiowej (cm)"**. Aplikacja "pod maską" mapuje te wartości na tabele rozmiarów (Karlovsky/Portwest), a wyliczony rozmiar trafia bezpośrednio do serwera wyszukiwarki. Dodatkowo wdrożono "Globalny Mutex" (`isSystemLocked`), który hardkorowo blokuje wszystkie przyciski przymierzalni podczas pracy VTON-u, likwidując problem masowych zapytań rodzących błąd HTTP 429 na architekturze *Serverless Vercel*. Awaryjne żółte paski o braku rozmiarów zastąpiono estetycznym, zielonym notyfikatorem sugerującym fallback inteligentny lub nadwymiar.
