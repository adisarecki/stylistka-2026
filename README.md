...
- **2026-03-01 (Refaktoryzacja IDM-VTON Schema + Hotfix UI Auth)**:
  ...
  - **Przełamywanie Maski IDM-VTON (Brutal Fallback + AI):** Przy napotkaniu sukni dążąc do zniwelowania uciętego tła na udach - to Gemini zaszywa w strumieniu parametr twardy: `, FULL LENGTH MAXI DRESS, COVERING LEGS ENTIRELY DOWN TO THE FLOOR, highly detailed`. Jeśli przesyłka AI zawiedzie, do akcji wkracza wbudowany w `api/try-on/` mechanizm *Brutal Fallback*. Zrzut ten skanuje zawartość payloadu wykrywając synonimy ("suknia", "maxi", "balowa") na polskiej stronie tekstu, nadpisując odgórniej kategorię `dresses`, stawiając `force_dc: true`, oraz doklejając siłowo wymuszający maskę nodz angielski rygor. Proces zabezpieczenia gwarantuje podawanie do generatorów wylącznie The English Tags a crop blokowany jest do okadrowania fałszywego.

- **2026-03-21 (Backendowy Portfel Kredytów VTON — Krok 4D-1 i Audyt 4D-1A)**:
  - **Atomowa Rezerwacja Kredytów:** Wdrożono backendowy portfel kredytów oparty o transakcje Firestore Admin (`user_credits/{uid}`). Każdy nowy zweryfikowany użytkownik otrzymuje dokładnie 3 bezpłatne kredyty przymiarki VTON.
  - **Ochrona FinOps i Granica Kosztowa:** Rezerwacja kredytu następuje atomowo po sprawdzeniu trwałego cache (Cache Hit kosztuje 0 kredytów) i PRZED jakimkolwiek wywołaniem Replicate. Brak środków zwraca natychmiast HTTP 402 `CREDIT_REQUIRED`.
  - **Idempotencja i Ledger Operacji:** Każde żądanie przymiarki posiada unikalny `requestId` (UUID v4) utrwalany w kolekcji `try_on_operations/{uid}_{requestId}` z wiązaniem parametrów (`cacheKey`). Zapobiega to wielokrotnemu naliczeniu przy ponowieniach lub równoległych żądaniach, a ponowne użycie tego samego `requestId` dla innych danych wejściowych zwraca HTTP 409 `IDEMPOTENCY_KEY_REUSED`.
  - **Durable Result Boundary & Refund Safety:** Granica trwałego wyniku oddziela wykonanie providera od zapisu w Storage i Firestore. Kredyt jest refundowany (`reserved -> refunded`) WYŁĄCZNIE, gdy błąd wystąpił przed trwale zapisanym wynikiem (`reservationCreated === true && creditFinalized === false && durableResultPersisted === false`). Operacja pozostaje reserved dla ochrony kosztowej. Automatyczna rekonsyliacja nie jest obecnie dostępna, ponieważ integracja replicate.run() nie zapisuje provider prediction ID. Stan wymaga przyszłego mechanizmu rozliczania albo ręcznej interwencji.
  - **Uwaga Techniczna dot. Rekonsyliacji i Znaczników Czasu:** Znaczniki czasu w transakcjach (`createdAt`, `updatedAt`) są wyliczane na podstawie czasu środowiska Vercel (`Date.now()`). Model IDM-VTON jest wywoływany synchronicznie przez `replicate.run(...)`, co nie udostępnia zewnętrznego `prediction_id` przed zakończeniem generacji. Twardy crash procesu Vercel w trakcie wywołania providera celowo pozostawia rezerwację w stanie `reserved` (zabezpieczenie FinOps przed wyciekiem darmowych generacji); pełna asynchroniczna rekonsyliacja ze statusem Replicate będzie możliwa po migracji na Replicate Predictions API / Webhooks.
