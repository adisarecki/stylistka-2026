...
- **Ominięcie SSL/403 Image Proxy & Strict Global Lock**: By pokonać mur blokad *Cloudflare* oraz *HTTP 403 Forbidden* wynikający z ochrony zaplecza graficznego potężnych dyskontów odzieżowych (Zalando, Mango etc.), VTON Endpoint `/api/try-on` asymiluje pliki zdalnie używając mechanizmu "Image Proxy". Node.js pobiera paczkę `ArrayBuffer` posługując się maską przeglądarki Next a następnie transformuje na wektor `Base64 Data URI` oddając go modelowi Replicate eliminując śledzenie po URL. Wdrożony we wcześniejszych etapach *Mutex* od dziś znany jako `isAppProcessing`, absolutnie twardo rygluje każde kliknięcie zapobiegając wpadkom rozproszeniowym (Kaskadowe 429 dla Replicate), a stare alerty braku rozmiarów wymarły ustępując prostej i suchej diagnozie: *"Dopasowaliśmy rozmiar [X]. Dostępne również: [X-1], [X+1]"*.
- **Zalando Size Engine z Fallbackiem +/- 1**: Czysty UI i koniec z domyślaniem się rozmiaru pracowniczego. W komponencie `TryOnWidget.tsx` usunięto klasyczny wybór rozmiarów na rzecz pola **"Obwód Klatki Piersiowej (cm)"**. Aplikacja mapuje te wartości na zaimplementowane referencyjne skale rozmiarów rynkowych znanych z portalu **Zalando** (klasyczna z buforem damskim od `34` do `64` oraz alfa męskim `XXS` do `5XL`). Wyliczony rozmiar trafia bezpośrednio do serwera, po czym aplikacja sprytnie proponuje rozmiary sąsiadujące ("rozmiar up" do większego zapasu tkaniny oraz "rozmiar down" na mocniejsze przyleganie). Zastąpiono przestarzałe żółte alerty estetycznym monitorem propozycji, gwarantując dostępność produktu dla odpytywanego sklepu z wybranym i polecanym parametrem przez AI.
