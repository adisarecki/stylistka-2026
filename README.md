...
- **2026-03-01 (Refaktoryzacja IDM-VTON Schema + Hotfix UI Auth)**:
  ...
  - **Przełamywanie Maski IDM-VTON (Długie Suknie):** Jeśli AI wykryje kategorię `dresses`, do opisu ubrania w modelu dopisywany jest rygorystyczny sufiks drukowanymi literami: `", FULL LENGTH MAXI DRESS, COVERING LEGS ENTIRELY DOWN TO THE FLOOR"`. Wymusza to na komponencie auto-maskującym przedłużenie generacji ubrania aż podłogę, omijając parametry sylwetki "krótkiej" ze zdjęcia referencyjnego użytkownika. Parametr `crop` podczas inferencji jest zablokowany na sztywno (`false`). W API znajduje się też jawny twardy log rzutu obiektu do Replicate, dostarczający inżynierom dokładny podgląd `replicatePayload` w logach Vercel.
