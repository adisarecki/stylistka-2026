...
- **2026-03-01 (Refaktoryzacja IDM-VTON Schema + Hotfix UI Auth)**:
  ...
  - **Przełamywanie Maski IDM-VTON (Długie Suknie & Strict English Tagging):** Jeśli pipeline wykryje kategorię `dresses`, całkowicie odcina dołączenie polskich rad stylistycznych (tzw. `bodyTypeModifier`) do opisu ubrania w Replicate, by model analizy obrazu nie oponował i nie gubił się w natywnym języku. Zamiast tego z twardej logiki backendu wklejany jest rygorystyczny sufiks drukowanymi literami: `", FULL LENGTH MAXI DRESS, COVERING LEGS ENTIRELY DOWN TO THE FLOOR, highly detailed"`. Wymusza to na komponencie auto-maskującym przedłużenie generacji ubrania aż podłogę, omijając parametry sylwetki "krótkiej" ze zdjęcia referencyjnego użytkownika. Parametr `crop` podczas inferencji jest zablokowany na sztywno (`false`). W API znajduje się też jawny twardy log rzutu obiektu do Replicate, dostarczający inżynierom dokładny podgląd `replicatePayload` w logach Vercel.
