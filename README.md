...
- **Zalando Size Engine z Fallbackiem +/- 1 & Global Concurrency Lock**: Czysty UI i koniec z domyślaniem się rozmiaru pracowniczego. W komponencie `TryOnWidget.tsx` usunięto klasyczny wybór rozmiarów na rzecz pola **"Obwód Klatki Piersiowej (cm)"**. Aplikacja mapuje te wartości na zaimplementowane referencyjne skale rozmiarów rynkowych znanych z portalu **Zalando** (klasyczna z buforem damskim od `34` do `64` oraz alfa męskim `XXS` do `5XL`). Wyliczony rozmiar trafia bezpośrednio do serwera, po czym aplikacja sprytnie proponuje rozmiary sąsiadujące ("rozmiar up" do większego zapasu tkaniny oraz "rozmiar down" na mocniejsze przyleganie). Zastąpiono przestarzałe żółte alerty estetycznym monitorem propozycji, gwarantując dostępność produktu dla odpytywanego sklepu z wybranym i polecanym parametrem przez AI. Całość działa w obiekcie ścisłej osłony antyspammingowej - tzw. `Global Mutex`, który unieruchamia przyciski w fazie pracy modelu blokując błędy HTTP 429 na architekturze _Serverless API_.
